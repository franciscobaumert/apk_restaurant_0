...
<?php
include("modal/registro_sala.php");
include("modal/editar_sala.php");
?>
        ...
        <div class="box box-success">
          <div class="box-header">
            <div class="btn-group pull-right">
              <button type='button' class="btn btn-info" data-toggle="modal" data-target="#nuevoProducto"><span class="glyphicon glyphicon-plus" ></span> Nueva Sala</button>
            </div>
            <h3 class="box-title">
              LISTA DE SALAS
            </h3>
...
<script>
		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'./ajax/buscar_salas.php?action=ajax',
		...
		{
			var q= $("#q").val();
		if (confirm("Realmente deseas eliminar la sala")){	
		$.ajax({
        type: "GET",
        url: "./ajax/buscar_salas.php",
        data: "id="+id,"q":q,
		 beforeSend: function(objeto){
 ...
 var parametros = $(this).serialize();
  ...
  event.preventDefault();
	...
	function obtener_datos(id){
			var nombre_sala = $("#nombre_sala"+id).val();
            //var des_cat = $("#des_cat"+id).val(); 
            $("#mod_cat").val(nombre_sala);
            //$("#mod_des").val(des_cat);
            $("#mod_id").val(id);
