	...
	if (empty($_POST['nombre'])) {
		...
		$query_check_user=mysqli_num_rows($query_check_user_name);
                if ($query_check_user == 1) { ?>
								...
								toastr.warning("Cliente duplicado","Precaucion");
								</script>
            <?php }else{
                    $sql="INSERT INTO clientes VALUES (NULL,'$nombre','$telefono','$email','$direccion','$estado','$date_added','$doc1','$dni','$ce','$ven','Peru','$departamento','$provincia','$distrito','$cuenta','1','$tienda1','0','0','0','$documento')";
                    $query_new_insert = mysqli_query($con,$sql);
                            if ($query_new_insert){ ?>
									...
									toastr.success("Cliente registrado","Exito");
									</script>
                            <?php } else{ ?>
                                    <script>
											toastr.options = {
