<?php
	...
	if (isset($_GET['id'])){
		...
		$numrows = $row2['numrows'];
          	...
          	if ($numrows==0 and $delete1=mysqli_query($con,$del1) and $delete2=mysqli_query($con,$del2) and $delete4=mysqli_query($con,$del4)){
                  $sql=mysqli_query($con, "select * from detalle_factura where folio='".$folio."' and numero_factura='".$numero_factura."' and ven_com=1 and tienda=$tienda and id_cliente=$id_cliente and tipo_doc=$tipo_doc" );
                    while ($row=mysqli_fetch_array($sql))
                    {
                        ...
                        if($id_producto>0){
                            $c=$b-$cantidad;
                            $insert=mysqli_query($con,"INSERT INTO detalle_factura VALUES (NULL,'$id_cliente','$usuario','$numero_factura','$cuenta2','$id_producto','$cantidad','0','$tienda1','0','2','$fecha1','0','$tipo_doc','$c','1','$folio')");  
            ...
            toastr.success("Comprobante anulado","Exito");
            </script>
			<?php 
		}else{
			?>
            ...
            toastr.error("No se puede anular el comprobante","Error");
            </script>
			<?php
	...
	if($action == 'ajax'){
		...
		$sTable = "facturas, clientes, users, mesas";
		$sWhere = "";
		$sWhere.=" WHERE facturas.id_cliente=clientes.id_cliente and facturas.tienda=$tienda1 and facturas.id_vendedor=users.user_id and facturas.ven_com=1 and facturas.estado_factura<=3 and facturas.activo=1 and facturas.numero_factura>0 and facturas.id_mesa=mesas.id_mesa and facturas.is_applied=1";
                if ( $_GET['q'] != "" )
		{
					...
					?>
                        ...
                        <td><?php echo $nombre_cliente;?></td>
                                                ...
                                                <!--<td><span class="label <?php echo $label_class;?>"><?php echo $text_estado; ?></span></td>-->
                                                 <td><a href="#" class='btn btn-primary btn-xs' title='Descargar pdf' onclick="imprimir_factura('<?php echo $id_factura;?>');"><i class="fa fa-file-pdf-o"></i></a> </td>
                                                 <td><a href="#" class='btn btn-primary btn-xs' title='Descargar ticket' onclick="imprimir_factura2('<?php echo $id_factura;?>');"><i class="fa fa-ticket"></i></a> </td>
                                                 <td class="text-right">
                    
                        <?php  
                                                if ($estado_factura1==1){
                                                    ?>
                                                <a href="guia.php?accion=<?php echo $id_factura;?>" class='btn btn-info btn-xs' title='Guia de remision' ><i class="fa fa-file-word-o"></i></a> 
                                                 <?php
                                                }   
                                                ?>

                                                <?php if ($estado_factura1==3) { ?>
                                                    <a href="editar_factura.php?id_factura=<?php echo $id_factura;?>" class='btn btn-default btn-xs' title='Editar factura' ><i class="glyphicon glyphicon-edit"></i></a> 
                                                <?php } ?>
                                                
                                                <a href="#" class='btn btn-warning btn-xs' title='Enviar correo' onclick="imprimir_factura1('<?php echo $id_factura;?>');"><i class="fa fa-envelope-o"></i></a> 
                        <?php  
                                                //if ($estado_factura1<=3 && $aceptado=="aceptada"){
                                                    //?>
                                                    <a href="#" class='btn btn-danger btn-xs' title='Dar de baja' onclick="eliminar('<?php echo $id_factura; ?>')"><i class="glyphicon glyphicon-trash"></i> </a>
                                                    <?php
                                               // }   
                                                ?>
                                                </td>
