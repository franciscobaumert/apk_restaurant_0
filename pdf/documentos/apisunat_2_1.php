	...
	public function enviar_documento($ruc, $usuario_sol, $pass_sol, $ruta_archivo, $ruta_archivo_cdr, $archivo, $ruta_ws) {
        ...
        if ($httpcode == 200) {
            ...
            if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) {
				...
				toastr.success("<?php echo $resp['mensaje']; ?>","Documento enviado");
				</script>
            ...
            <?php } else {
				...
				toastr.error("<?php echo $resp['mensaje']; ?>","Documento rechazado");
				</script>
			...
			//echo '<strong>No responde la web. Verifique su conexión a internet e inténtelo nuevamente!</strong>.<br>';
			//echo '<span>si el problema persiste comuníquese con soporte.</span>';
			//echo '</div>';
			//echo '</div>';
			//echo '</div>';

            //echo "No responde la web. Verifique su conexión a internet e inténtelo nuevamente, si el problema persiste comuníquese con soporte.";
            $resp['respuesta'] = 'errorc';
            $resp['cod_sunat'] = "0000";
            $resp['mensaje'] = "<br />Error de conexion. <br /> Verifique su conexion a internet e intentelo nuevamente. si el problema persiste comuniquese con el area de soporte.<br />";
			...
			toastr.warning("Verifique su conexión a internet e inténtelo nuevamente.","No responde la web");
			</script>
        <?php }
        return $resp;
    }
